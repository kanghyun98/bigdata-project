...
```{r}
BigRes <- lmG3$residuals %>% sort(decreasing = T) %>% names()

BigResIDX <- BigRes[1:ceiling(length(BigRes)*0.001)]

taxiG3[BigResIDX,] %>% mutate(estimateFare = lmG3$fitted.values[BigResIDX], residual = lmG3$residuals[BigResIDX]) %>% select(total_amount, estimateFare, residual) %>% head(50)
taxiG3[BigResIDX,] %>% mutate(estimateFare = lmG3$fitted.values[BigResIDX], residual = lmG3$residuals[BigResIDX]) %>% select(total_amount, estimateFare, residual) %>% tail(50)
```
...
# 변수가 다 있는 분포에서 살펴보자
# rm(lmG1, lmG3, lmG3)
selectedDataG3 <- taxiG3[BigResIDX, ]

selectedDataG3 %>% glimpse()
selectedDataG3 %>% head()
```

```{r}
# VendorID : 특정 업체에서 유독 많다
selectedDataG3 %>% select(vendor_id) %>% table()
nOfObs <- selectedDataG3 %>% group_by(vendor_id) %>% summarize(count = n())
...
ggplot(nOfObs,
  ...
  labs(x = "vendor ID",
       y = "count",
       title = "vendor ID",
       subtitle = "rate code 1, high distance") +
...
BadDrivers <- selectedDataG3 %>% select(hack_license, total_amount) %>% mutate(estimateFare = lmG3$fitted.values[BigResIDX]) %>% group_by(hack_license) %>% summarize(totalSum = sum(total_amount), estimateSum = sum(estimateFare), count = n()) %>% mutate(ratio = totalSum/estimateSum) %>% arrange(desc(count))
...
allDrivingFreqG3 <- taxiG3 %>% select(hack_license) %>% group_by(hack_license) %>% summarize(count = n())
bigResDrivingFreaG3 <- selectedDataG3 %>% select(hack_license) %>% group_by(hack_license) %>% summarize(count=n())
allDrivingFreqG3 %>% dim()
bigResDrivingFreaG3 %>% dim()
count <- inner_join(allDrivingFreqG3, bigResDrivingFreaG3, by = "hack_license", suffix = c("All", "Outlier")) %>% arrange(desc(countOutlier))
BadDriversDone <- bind_cols(BadDrivers, count[,"countAll"])[, c("hack_license","totalSum", "estimateSum", "ratio", "count", "countAll")]
BadDriversDone %>% arrange(desc(count))
...
```{r}
# datetime : 특정시간대인지
pickupHourG3 <- hour(selectedDataG3$pickup_datetime) %>% table() %>% as.data.frame()
names(pickupHourG3) <- c("Hour", "count")

dropoffHourG3 <- hour(selectedDataG3$dropoff_datetime) %>% table() %>% as.data.frame()
names(dropoffHourG3) <- c("Hour", "count")

hourG3 <- inner_join(pickupHourG3, dropoffHourG3, by="Hour", suffix = c("Pickup","Dropoff"))
hourG3 <- hourG3 %>% pivot_longer(cols = !Hour, names_to = "hourType", values_to = "value")
# Change line colors by groups
ggplot(hourG3, aes(x=Hour, y=value, group=hourType)) +
...
```{r}
# Passenger count : 특정 승객수인지
NofObs <- selectedDataG3 %>% group_by(passenger_count) %>% summarize(count=n())
...
```{r}
g1 <- selectedDataG3 %>% ggplot(aes(x = pickup_longitude, y = pickup_latitude)) +
  geom_hex(bins=100) +
  scale_fill_continuous(type = "viridis") +
  labs(title = "승차 경도 및 위도") +
  theme_bw()

g2 <- selectedDataG3 %>% ggplot(aes(x = dropoff_longitude, y = dropoff_latitude)) +
  geom_hex(bins=100) +
  scale_fill_continuous(type = "viridis") +
...
g3 <- ggmap(NYMAP) + geom_point(data = selectedDataG3, 
                              aes(x = pickup_longitude, y = pickup_latitude), size =.003, alpha =.1) +
  labs(title = "Taxi PickUp location")
g3

g4 <- ggmap(NYMAP) + geom_point(data = selectedDataG3, 
                              aes(x = dropoff_longitude, y = dropoff_latitude), size =.003, alpha=.1) +
...
```{r}
# Payment type : 특정 지불 방식인지
NofObs <- selectedDataG3 %>% group_by(payment_type) %>% summarize(count=n())
...
ggplot(NofObs,
  ...
  labs(x = "payment type",
       ...
       title = "payment type",
       fill = "payment type",
       subtitle = "rate code 1, long distance") +
  geom_text(aes(label = count), vjust=0) +
  theme_bw()
...
selectedDataG3[-which(selectedDataG3$tip_amount == 0),] %>% mutate(tipratio = tip_amount / total_amount) %>% select(tipratio) %>% skim()


# cluster 1 (전체데이터) 에서 팁의 비율 (=일반적인 팁의 비율, 약 15%)
taxiG3[-which(taxiG3$tip_amount == 0),] %>% mutate(tipratio = tip_amount / total_amount) %>% select(tipratio) %>% skim()

# 전체 데이터에서의 tip Ratio가 15%인데, 이상치로 판별된 데이터 셋의 tip Ratio는 15%보다 한참 큰 32.3% 이다(평균, tip=0 제외).
